...
def power_method_my(MPO: mpo.myMPO, startMPS, pm_params, HMPO = 0 ):
    ...
    progress_bar = tqdm(range(1,iter_max+1))
    # for j in tqdm(range(1,iters)):
    for jj in progress_bar:
        oPsi = mpomps.applyMPOtoMPS(MPO, oPsi)  
        oPsi.bringCan(chiMax = chi_max)
    ...
    print(f"max chi reached(/max): {max_chi_reached}/{chi_max}, final dSmid = {devec[-1]}")
    ...
    #return oPsi, iter, Svec, devec, energies, max_chi_reached, chi_max
    return ents, max_chi_reached, curr_chi, oPsi, energies 
